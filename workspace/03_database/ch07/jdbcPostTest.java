...
//        findAll();
    ...
    static void findAll(){
        ...
        try{ // 플랜 A
            ...
            rs = stmt.executeQuery("select id, title, view_count as viewCount, created_at as createdAt FROM  post");
            // 결과 커서를 밑으로 내리는 작업
            while(rs.next()){
                int id = rs.getInt("id");
    ...
    static void findById(int id){
        // import 필요
        String sql = "Select id, title, view_count as viewCount, created_at as createdAt FROM post where id = " + id;
        Connection conn = null;
        Statement stmt = null;
        ResultSet rs = null;
        ...
        try{ // 플랜 A
            ...
            // 3. SQL 실행 (SELECT)
            //4. 결과 수신 (ResultSet 객체 생성)
            rs = stmt.executeQuery("select id, title, content, view_count as viewCount, created_at as createdAt FROM  post");
            // 결과 커서를 밑으로 내리는 작업
            while(rs.next()){
                String title = rs.getString("title");
